...
SET VARS=WSL_DISTRIBUTION_NAME=%WSL_DISTRIBUTION_NAME% YOCTO_SDK_INSTALL=%YOCTO_SDK_INSTALL% YOCTO_SDK_ENV_SCRIPT=%YOCTO_SDK_ENV_SCRIPT%

WSL -d %WSL_DISTRIBUTION_NAME% --cd %CURRENT_DIRECTORY% bash -c "dos2unix $(wslpath '%BSP_ROOT_DIRECTORY%')/vee/scripts/*.sh && %VARS% bash $(wslpath '%BSP_ROOT_DIRECTORY%')/vee/scripts/build.sh"
