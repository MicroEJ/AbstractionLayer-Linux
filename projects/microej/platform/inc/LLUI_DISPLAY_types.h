...
typedef enum DRAWING_Incident {
	 ...
	 * No exception will be thrown if this flag is not set, although other flags will keep their state and be readable
	 * in the application.
 ...
 * - all functions that use a jbyte format: LLUI_DISPLAY_IMPL_getDrawerIdentifier(), LLUI_DISPLAY_isDisplayFormat(),
 * etc.
 ...
 * This jbyte array is retrieved in MicroEJ application using the method Image.getSNIContext().
 *
 ...
 * array is retrieved in MicroEJ application using the method GraphicsContext.getSNIContext().
 *
...
#endif
#endif // ifndef _LLUI_DISPLAY_TYPES
